...
app.post("/api/easebuzz/success", async (req, res) => {
    ...
    try {
        ...
        const frontendUrl = `http://localhost:5500/atd_archana_sava/payment-status.html?status=success&txnid=${encodeURIComponent(txnid)}`;
...
app.post("/api/easebuzz/failure", async (req, res) => {
    ...
    try {
        ...
        const frontendUrl = `http://localhost:5500/atd_archana_sava/payment-status.html?status=failed&txnid=${encodeURIComponent(txnid)}`;
        return res.redirect(frontendUrl);
    } catch (err) {
        console.error(err);
        res.status(500).send("Error updating Airtable");
